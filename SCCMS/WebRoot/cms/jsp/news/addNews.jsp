...
<input type="hidden" name="id" id="newsId" value="${news.id }"/>
<table width="100%" border="0" align="center" cellpadding="5" cellspacing="0">
	<tr>
    	<td>资讯标题</td>
        <td valign="middle">
            <input id="title" type="text" name="news.title" class="l-text" style="width:400px;" value="${news.title}" onblur="checkNull(this,$('#msg'));">
        </td>
    </tr>
    <tr>
    <tr>
        ...
        <td>
            <textarea name="content" cols="100" rows="5" class="l-textarea" id="editor" style="width:651px"  >${news.content}</textarea>
            <textarea name="news.content" id="content" style="display:none"></textarea>
        </td>
    </tr>
    <tr>
...
function commit()
{
	...
	$.post(
			"${pageContext.request.contextPath}/news/addOrUpdate",
			{"news.id":$("#newsId").val(),"news.title":$("#title").val(),"news.content":editor.getContent()},
			function(data){
				if(data)
				{
					alert("操作成功");
					parent.parent.closeTab("addNews");
				}else
				{
					$.ligerDialog.alert("操作失败,该记录可能不存在",function(){
						if(status=="") parent.closeTabAndFresh("addNews");
						else {
