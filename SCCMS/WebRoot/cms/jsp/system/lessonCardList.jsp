  ...
  <link href="${pageContext.request.contextPath}/cms/css/sliver/all.css" rel="stylesheet" type="text/css" /> 
    ...
    <script type="text/javascript">
        ...
        function f_showGrid()
        {
                ...
                {display: '使用时间',name:'cardUseTime',minWidth:150 ,type:"mydate"},
                {display: '包含课程',name:'className',minWidth:150}
                ], url:"${pageContext.request.contextPath}/package/cardList?num="+n, pageSize: 20, sortName: 'id',sortOrder:'desc',method:"post",format:"yyyy-MM-dd",
        		...
        		$.ligerDialog.confirm("确定不导出当次生成的吗？",function(r)
        			{
        				if(r)
        				{
        					parent.overrideTab("createLessonCards",'${pageContext.request.contextPath}/cms/jsp/system/createLessonCards.jsp','批量生成');
        				}
        			}
        		);
        		return;
	        }
	       parent.addTab("createLessonCards",'${pageContext.request.contextPath}/cms/jsp/system/createLessonCards.jsp','批量生成');
        ...
        function f_downLoad()
        {
        	if(n!=0)
        	{
        		location="${pageContext.request.contextPath}/package/excel?num="+n;
        	}else
        	{
        		parent.addTab("exportLessonCard","${pageContext.request.contextPath}/cms/jsp/system/exportLessonCard.jsp","导出");
        	}
        }
        //搜索
        function f_search()
        {
        	...
        	if(!/^[0-9A-Za-z]+[-]?[0-9]*$/.test($.trim($("#content").val())))
        	{
        		alert("请输入正确的卡号");
        		return;
        	}
            ...
            g.set({url:"${pageContext.request.contextPath}/package/searchCard?str="+str+"&searchName="+$("#search").val()});
        }
        //返回
        function f_back()
        {
        	$("#back").hide();
        	g.set({url:"${pageContext.request.contextPath}/package/cardList?num="+n});
        	//g.loadData();
        }
    </script>
</head>
