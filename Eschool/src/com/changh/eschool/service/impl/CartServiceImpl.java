...
public class CartServiceImpl implements CartService, Serializable {
	...
	public boolean add(int productId, int pType) throws Exception {
		...
		for (CartItem item : items) {
			if (item.getItem().getProductId() == productId
					&& item.getItem().getItemPType() == pType) {
				// 已经购买了就不再往车里加
				item.setBuy(true);
				return false;
			}
		...
		if (pType == Constant.SINGER_GRADE) {
			...
			if (grade != null && grade.getGradeSingle() == 0) {
				...
				item.setItemName(gradeDao.findExamName(productId) + "："
						+ grade.getName());
			...
			if (cp != null) { // 2013.05.24修改
				...
				item.setItemName(classPackageDao.findExamName(productId) + "："
						+ cp.getPkgName());
	...
	public double getSavedMoney() {
		...
		for (CartItem item : items) {
			if (item.isBuy()) {
				money = money
						+ (item.getItem().getItemOPrice() - item.getItem()
								.getItemRPrice());
			}
		}
		return money;
	...
	// 从cookie里读数据
	public List<CartItem> getProFromCookie(Cookie packageCookie,
			Cookie gradeCookie) throws Exception {
		// 套餐
